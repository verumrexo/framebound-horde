...
- there are no waves, lanes, gravity, bouncing, pachinko physics, build pauses, or active-enemy cap. the run-wide settings chosen at deployment are the horde pace, a time stretch on the threat clock rather than a difficulty preset, and the rift layout: authored entries, or a seeded shuffle that moves every rift to a random open edge and permutes the unlock order so entries and their timing stay unpredictable. both are carried by the host-validated start and restart commands and survive corrections. if the player cannot clear the horde, the population is allowed to bury the base. above 20,000 movement records, nearby 1 hp enemies begin sharing deterministic swarm packets; this limits simulation records rather than gameplay population.
