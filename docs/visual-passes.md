...
- `catalogDefinition(snapshot, id)` indexes the catalog once per snapshot instead of a linear scan per tower per frame. the never-pushed knot/backwash flash branches are gone. `drawReworkedCombat` stays: `state.turretRework` is only cleared between runs and `turret-rework.js` recreates it every tick, so broadside pellets, flechette nails, cyclone orbits and the reworked control shots all draw through it; heads now use the combat-mark proportions, and `npm run test:visual` feeds every shot type through the production route at every zoom.
